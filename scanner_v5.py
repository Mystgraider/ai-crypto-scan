...
def main():
    ...
    for symbol in symbols:
        if _time.time() - _scan_start_time > _scan_time_budget_sec:
            print(f"      ⏱️  Time budget ({_scan_time_budget_sec}s) reached — stopping early with {len(candidates)} candidate(s) found so far, {symbols.index(symbol)}/{len(symbols)} symbols processed.")
        ...
        try:
            ...
            live = df_1h.iloc[-1]
            prev = df_1h.iloc[-2]
            req = ["close","ema_20","ema_50","atr","adx","rsi","roc","rel_volume","macd","macd_sig","macd_hist","bb_pct_b","stoch_k","stoch_d"]
            if prev[req].isnull().any():
                skip["errors"] += 1
                continue
            price = float(live["close"])
            ema20 = float(prev["ema_20"]); ema50 = float(prev["ema_50"]); atr = float(prev["atr"])
            adx = float(prev["adx"]); rsi = float(prev["rsi"]); roc = float(prev["roc"]); rel_volume = float(prev["rel_volume"])
            macd = float(prev["macd"]); macd_sig = float(prev["macd_sig"]); macd_hist = float(prev["macd_hist"])
            bb_pct_b = float(prev["bb_pct_b"]); stoch_k = float(prev["stoch_k"]); stoch_d = float(prev["stoch_d"])

            trend = trend_engine.analyze(price=price, ema20=ema20, ema50=ema50, adx=adx, roc=roc, macd=macd, macd_sig=macd_sig, macd_hist=macd_hist, stoch_k=stoch_k, stoch_d=stoch_d, bb_pct_b=bb_pct_b)
            trend_score = trend["score"]
            if CONFIG.get("require_trend_gate", True):
                if trend["direction"] == "NONE":
                    skip["trend"] += 1; _trace(symbol, "trend_none_and_gate_required"); continue
                candidate_directions = [trend["direction"]]
            else:
                candidate_directions = ["LONG", "SHORT"] if trend["direction"] == "NONE" else [trend["direction"]]
            ...
            for direction in candidate_directions:
                ...
                if CONFIG["btc_filter_enabled"]:
                    if direction == "LONG" and not btc_regime["allow_long"]:
                        skip["btc"] += 1; _trace(symbol, "btc_regime_block", direction=direction, regime=btc_regime.get("regime")); continue
                ...
                if CONFIG.get("pause_shorts", True) and direction == "SHORT":
                    skip["btc"] += 1; _trace(symbol, "btc_regime_block", direction=direction, regime=btc_regime.get("regime")); continue
                if btc_regime["regime"] == "RANGE" and trend["direction"] != "NONE" and effective_trend_score < CONFIG.get("range_regime_min_score", 85):
                    skip["btc"] += 1; _trace(symbol, "btc_regime_block", direction=direction, regime=btc_regime.get("regime")); continue
                ...
                if CONFIG["funding_enabled"]:
                    ...
                    if direction == "LONG" and not funding_result["long_ok"]:
                        skip["funding"] += 1; _trace(symbol, "funding_block", direction=direction); continue
                ...
                if CONFIG["beta_filter_enabled"] and direction == "SHORT":
                    beta_result = beta_filter.evaluate(symbol=symbol, direction=direction, coin_closes=df_1h["close"].tolist(), btc_closes=btc_closes)
                    if not beta_result["short_ok"]:
                        skip["high_beta"] += 1; _trace(symbol, "beta_block", direction=direction); continue

                sr_levels = sr_engine.find_levels(df_1h)
                sr_bonus = sr_engine.score_bonus(direction, sr_levels)
                bb_width_pctile = float(df_1h["bb_width_pctile"].iloc[-2]) if "bb_width_pctile" in df_1h.columns else 1.0
                ...
                try:
                    ...
                    if len(_df_rrce_15m) >= 20 and len(_df_rrce_5m) >= 20:
                        ...
                        patience_bars = CONFIG["rrce_range_patience_bars"] if is_range_regime else CONFIG["rrce_default_patience_bars"]
                        rrce_result = active_rrce_engine.evaluate(df_htf=_df_rrce_15m, df_mtf=_df_rrce_15m, df_ltf_confirm=_df_rrce_5m, df_ltf_exec=_df_rrce_5m, direction=direction, price=price, patience_bars=patience_bars)
                ...
                try:
                    ...
                    if len(_exp_4h) >= 20 and len(_exp_1h) >= 20:
                        _exp_result = experiment_4h_engine.evaluate(df_htf=_exp_4h, df_mtf=_exp_4h, df_ltf_confirm=_exp_1h, df_ltf_exec=_exp_1h, direction=direction, price=price)
                        experiment_4h_log.append({"symbol": symbol, "direction": direction, "valid": _exp_result.get("valid"), "failed_at": _exp_result.get("failed_at"), "stage1_position_pct": (_exp_result.get("stage1") or {}).get("position_pct")})
                except Exception as _exp_e:
                    experiment_4h_log.append({"symbol": symbol, "direction": direction, "error": str(_exp_e)})

                if rrce_result:
                    s1_data = rrce_result.get("stage1"); s2_data = rrce_result.get("stage2"); s3_data = rrce_result.get("stage3")
                    if s1_data and s1_data.get("passed"): skip["rrce_stage1_passed"] += 1
                    if s2_data:
                        pools = s2_data.get("pools")
                        if pools is not None: stage2_pool_counts.append(len(pools))
                        if s2_data.get("passed"):
                            skip["rrce_stage2_passed"] += 1
                            try:
                                watchlist_count = record_stage2_setup(symbol=symbol, direction=direction, regime=btc_regime["regime"], stage1=s1_data, stage2=s2_data, ttl_hours=CONFIG["rrce_watchlist_hours"])
                    ...
                    if s3_data and s3_data.get("passed"): skip["rrce_stage3_passed"] += 1
                    if rrce_result.get("valid"): skip["rrce_stage4_valid"] += 1
                ...
                if not rrce_result or not rrce_result.get("valid"):
                    ...
                    skip[f"rrce_fail_{fail_stage}"] = skip.get(f"rrce_fail_{fail_stage}", 0) + 1
                    if fail_stage == "stage1_range" and rrce_result:
                        s1_data = rrce_result.get("stage1")
                        if s1_data and "position_pct" in s1_data: stage1_position_samples.append(s1_data["position_pct"])
                ...
                rrce_risk = rrce_engine.live_entry_levels(direction=direction, live_price=price, stage4=rrce_result["stage4"], max_deviation_pct=CONFIG["rrce_entry_max_deviation_pct"], min_rr=CONFIG["min_rr"])
                if not rrce_risk["valid"]:
                    if rrce_risk["reason"] == "price_away_from_rrce_entry": skip["rrce_entry_away"] += 1
                    else: skip["rrce_entry_invalid"] += 1
                    _trace(symbol, "rrce_entry_block", direction=direction, reason=rrce_risk["reason"])
                    continue
                ...
                if direction == "SHORT" and CONFIG["short_requires_resistance"]:
                    if not sr_engine.short_has_ceiling(sr_levels, CONFIG["short_resistance_max_pct"]):
                        skip["sr_no_ceil"] += 1; _trace(symbol, "sr_no_ceiling", direction=direction); continue

                coin_closes = df_1h["close"].tolist()
                rs = rs_engine.calculate(coin_closes, btc_closes) if len(btc_closes) >= 21 else {"rs_score": 50.0, "rs_label": "NEUTRAL", "rs_ratio": 1.0}
                if rs["rs_label"] == "WEAK":
                    skip["weak_rs"] += 1; _trace(symbol, "weak_rs", direction=direction); continue
                ...
                if rel_volume > vol_max:
                    skip["quality"] += 1; _trace(symbol, "quality_block", direction=direction); continue
                spike = spike_engine.analyze(rel_volume)

                mtf_status = "SKIPPED"; mtf_multiplier = 1.0; mtf_4h_dir = "UNKNOWN"; mtf_15m_dir = "UNKNOWN"
                    ...
                    try:
                        ...
                        if _df_15m_raw is not None:
                            ...
                            if len(df_15m) >= 2 and not df_15m.iloc[-2][["ema_20","ema_50","adx"]].isnull().any():
                                trend_15m = mtf_engine.analyze_15m(df_15m); mtf_15m_dir = trend_15m["direction"]
                    except Exception:
                        trend_15m = None
                    for _attempt in range(2):
                        try:
                            _df_4h_raw = market_loader.get_4h(symbol); df_4h = Indicators.apply(_df_4h_raw)
                            if len(df_4h) >= 2 and not df_4h.iloc[-2][["ema_20","ema_50","adx"]].isnull().any():
                                tf4 = mtf_engine.analyze_4h(df_4h); mtf_4h_dir = tf4["direction"]
                                confirm = mtf_engine.confirm(direction, tf4, trend_15m)
                                mtf_status = confirm["status"]; mtf_multiplier = confirm["multiplier"]; break
                        except Exception:
                            if _attempt == 1:
                                if adx >= 30: mtf_status = "ALLOWED"; mtf_multiplier = 0.95; mtf_4h_dir = "PROXY"
                                else: mtf_status = "SKIPPED"; mtf_multiplier = 1.0
                if CONFIG["mtf_reject_counter_trend"] and mtf_status in ("REJECTED", "SKIPPED"):
                    skip["mtf"] += 1; _trace(symbol, "mtf_block", direction=direction); continue

                quality_score = quality_engine.score(rel_volume=rel_volume, rsi=rsi, direction=direction, stoch_k=stoch_k, bb_pct_b=bb_pct_b, macd_hist=macd_hist)
                effective_quality_score = quality_score
                if not CONFIG.get("require_quality_engine", True) and quality_score < CONFIG["min_score"]: effective_quality_score = CONFIG["min_score"]
                ...
                if CONFIG["oi_enabled"]:
                    try:
                        ...
                        if oi_data["available"]:
                            ...
                            oi_result = oi_engine.analyze(current_oi=oi_data["current_oi"], previous_oi=oi_data["previous_oi"], price_change=price_chg, direction=direction)
                    except Exception:
                        pass
                risk = rrce_risk
                if not validator.validate(direction, effective_trend_score, effective_quality_score, risk):
                    if risk is None:
                        skip["risk"] += 1; _trace(symbol, "risk_none", direction=direction)
                    else:
                        skip["quality"] += 1; _trace(symbol, "quality_block", direction=direction)
                    continue
                ...
                composite = min(100.0, max(0.0, round((base + sr_bonus + oi_adj + fund_adj + squeeze_bonus + vp_bonus + rrce_bonus) * mtf_multiplier, 2)))
                g = grade_score(composite)
                if g == "D":
                    skip["d_grade"] += 1; _trace(symbol, "d_grade_block", direction=direction); continue
                if composite >= CONFIG.get("signal_score_ceiling", 84):
                    skip["overextended"] = skip.get("overextended", 0) + 1; continue

                _trace(symbol, "CANDIDATE_FOUND", direction=direction)
                candidates.append({
                    "symbol": symbol, "direction": direction,
                    "trend_score": effective_trend_score, "quality_score": effective_quality_score,
                    "rs_score": rs["rs_score"], "rs_label": rs["rs_label"], "rs_ratio": rs.get("rs_ratio", 1.0),
                    "sr_bonus": sr_bonus, "sr_support": sr_levels["nearest_support"], "sr_resistance": sr_levels["nearest_resistance"],
                    "composite": composite, "grade": g, "rr": risk["rr"], "entry": risk["entry"], "sl": risk["sl"],
                    "tp1": risk["tp1"], "tp2": risk["tp2"], "tp3": risk["tp3"],
                    # Keep the structural Stage-4 plan so the alert-time price
                    # check can recompute entry/RR/targets instead of mutating
                    # only the entry while leaving stale risk levels behind.
                    "_rrce_stage4": rrce_result["stage4"],
                    "rsi": round(rsi, 2), "adx": round(adx, 2), "rel_volume": round(rel_volume, 2), "spike_tier": spike["tier"],
                    "macd_hist": round(macd_hist, 6), "stoch_k": round(stoch_k, 2), "bb_pct_b": round(bb_pct_b, 3),
                    "mtf_status": mtf_status, "mtf_4h": mtf_4h_dir, "mtf_15m": mtf_15m_dir,
                    "btc_regime": btc_regime["regime"], "funding_pct": str(funding_result.get("funding_pct", 0)),
                    "funding_pct_raw": funding_result.get("funding_pct_raw", 0.0), "oi_signal": oi_result["oi_signal"],
                    "beta_label": beta_result.get("beta_label", "N/A"),
                })
        except Exception as e:
            skip["errors"] += 1
            if "Insufficient candles" not in str(e) and "NaN" not in str(e): print(f"  ⚠️  {symbol}: {e}")

    print(f"      ✅ {len(candidates)} candidate(s)")
    print(f"      📊 cd:{skip['cooldown']} dated:{skip['dated']} btc:{skip['btc']} trend:{skip['trend']} fund:{skip['funding']} beta:{skip['high_beta']} mtf:{skip['mtf']} sr:{skip['sr_no_ceil']} rs:{skip['weak_rs']} q:{skip['quality']} risk:{skip['risk']} d:{skip['d_grade']} overext:{skip['overextended']} rrce:{skip['rrce_invalid']} rrce_away:{skip['rrce_entry_away']} budget_stop:{skip['time_budget_stop']} err:{skip['errors']}")

    try:
        import json as _json
        from datetime import datetime as _dt, timezone as _tz
        debug_row = {
            "ts": _dt.now(_tz.utc).isoformat(), "symbols_scanned": len(symbols), "candidates": len(candidates),
            "btc_regime_label": btc_regime.get("regime"), "btc_regime_allow_long": btc_regime.get("allow_long"), "btc_regime_allow_short": btc_regime.get("allow_short"),
            "btc_regime_adx": btc_regime.get("adx"), "btc_regime_rsi": btc_regime.get("rsi"), "btc_regime_reason": btc_regime.get("reason"),
            "stage1_position_pct_count": len(stage1_position_samples), "stage1_position_pct_min": round(min(stage1_position_samples), 1) if stage1_position_samples else None,
            "stage1_position_pct_max": round(max(stage1_position_samples), 1) if stage1_position_samples else None, "stage1_position_pct_avg": round(sum(stage1_position_samples)/len(stage1_position_samples), 1) if stage1_position_samples else None,
            ...
            "rrce_watchlist_active": active_count(CONFIG["rrce_watchlist_hours"]), **skip,
        }
        with open("storage/scan_debug_log.jsonl", "a") as f: f.write(_json.dumps(debug_row) + "\n")
    except Exception as _e: print(f"      ⚠️  debug log write failed: {_e}")

    try:
        exp_valid = sum(1 for r in experiment_4h_log if r.get("valid"))
        exp_row = {"ts": _dt.now(_tz.utc).isoformat(), "total_checked": len(experiment_4h_log), "valid_count": exp_valid, "results": experiment_4h_log}
        with open("storage/experiment_4h_log.jsonl", "a") as f: f.write(_json.dumps(exp_row) + "\n")
    except Exception as _e: print(f"      ⚠️  experiment log write failed: {_e}")

    try:
        trace_row = {"ts": _dt.now(_tz.utc).isoformat(), "trace": symbol_trace_log}
        with open("storage/symbol_trace_log.jsonl", "a") as f: f.write(_json.dumps(trace_row) + "\n")
    except Exception as _e: print(f"      ⚠️  symbol trace write failed: {_e}")
    ...
    for sig in ranked:
        try:
            ...
            if drift_pct > max_drift:
                print(f"  ⏭️  {sig['symbol']} {sig['direction']} skipped — price drifted {drift_pct:.2f}% since detection (entry {sig['entry']} → now {fresh_price})")
                continue

            # Phase 1C: revalidate the COMPLETE RRCE execution package at
            # the fresh price. Never change entry alone while keeping the
            # old SL/TP/RR, because that can silently create a different
            # risk profile from the one that was actually validated.
            live_risk = rrce_engine.live_entry_levels(
            ...
            if not live_risk["valid"]:
                print(f"  ⏭️  {sig['symbol']} {sig['direction']} skipped — live RRCE revalidation failed: {live_risk.get('reason')}")
                continue
            ...
            print(f"  ⚠️  {sig['symbol']} live price/RRCE revalidation failed ({e}) — skipping signal")
            continue

        confidence = conf_eng.estimate(trend_score=sig["trend_score"], quality_score=sig["quality_score"], historical_wr=hist_wr)
        sizing = sizer.calculate(grade=sig["grade"], confidence=confidence, entry=sig["entry"], sl=sig["sl"])

        mtf_icon = {"CONFIRMED_STRONG": "✅✅", "CONFIRMED": "✅", "ALLOWED": "🟡", "ALLOWED_WEAK": "🟠", "SKIPPED": "⬜"}.get(sig["mtf_status"], "⬜")
        btc_icon = {"BULL":"🟢","BEAR":"🔴","RANGE":"🟡","BULL_CAUTION":"🟡","BEAR_CAUTION":"🟡","EXTREME_BULL":"🔥","EXTREME_BEAR":"🧊"}.get(sig["btc_regime"],"⬜")
        rs_icon = {"STRONG":"💪","NEUTRAL":"➡️"}.get(sig["rs_label"],"➡️")
        key_level = ""
        if sig["direction"] == "SHORT" and sig["sr_resistance"]: key_level = f"\n🔒 Resistance: <code>{sig['sr_resistance']}</code>"
        elif sig["direction"] == "LONG" and sig["sr_support"]: key_level = f"\n🛡 Support: <code>{sig['sr_support']}</code>"
        message = format_signal(symbol=sig["symbol"], direction=sig["direction"], score=sig["composite"], entry=sig["entry"], sl=sig["sl"], tp1=sig["tp1"], tp2=sig["tp2"], tp3=sig["tp3"], rr=sig["rr"], grade=sig["grade"])
        message += (f"{key_level}\n\n" f"📊 <i>ADX:{sig['adx']} | RSI:{sig['rsi']} | StochK:{sig['stoch_k']} | Vol:{sig['rel_volume']}x ({sig['spike_tier']})</i>\n" f"📈 <i>MACD hist:{sig['macd_hist']} | BB%B:{sig['bb_pct_b']}</i>\n" f"{mtf_icon} MTF: <i>{sig['mtf_4h']} 4H / {sig['mtf_15m']} 15M ({sig['mtf_status']})</i>\n" f"{btc_icon} BTC: <i>{sig['btc_regime']}</i>\n" f"{rs_icon} RS: <i>{sig['rs_label']} ({sig['rs_ratio']}x BTC)</i>\n" f"💸 Funding: <i>{sig['funding_pct']}%</i> | " f"OI: <i>{sig['oi_signal']}</i> | " f"Beta: <i>{sig['beta_label']}</i>\n" f"🤖 Confidence: <b>{confidence}%</b>\n\n" f"{sizer.format_recommendation(sizing)}")
        send_telegram_alert(message)
        save_signal(symbol=sig["symbol"], direction=sig["direction"], entry=sig["entry"], sl=sig["sl"], tp1=sig["tp1"], tp2=sig["tp2"], tp3=sig["tp3"], score=sig["composite"], grade=sig["grade"], rr=sig["rr"], adx=sig["adx"], rsi=sig["rsi"], rel_volume=sig["rel_volume"], spike_tier=sig["spike_tier"], mtf_status=sig["mtf_status"], btc_regime=sig["btc_regime"], rs_label=sig["rs_label"], funding_pct=sig["funding_pct"], oi_signal=sig["oi_signal"], beta_label=sig["beta_label"])
        set_cooldown(sig["symbol"])
        print(f"  ✅ {sig['symbol']} {sig['direction']} | Grade:{sig['grade']} Score:{sig['composite']} | RSI:{sig['rsi']} StochK:{sig['stoch_k']} Vol:{sig['rel_volume']}x | MTF:{sig['mtf_status']} BTC:{sig['btc_regime']}")
